...
Использует Polza.ai (российский агрегатор LLM с OpenAI-совместимым API).
Хранит историю диалога в БД.
'''
import json
import os
...
def call_polza(messages: List[Dict[str, str]], api_key: str) -> str:
    '''Запрос к Polza.ai (российский агрегатор LLM с OpenAI-совместимым API).
    Документация: https://polza.ai
    Base URL: https://api.polza.ai/api/v1 — формат /chat/completions как у OpenAI.
    '''
    payload = {
    ...
    try:
        ...
        raise RuntimeError(f'Polza.ai {e.code}: {err[:200]}')
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        api_key = os.environ.get('POLZA_API_KEY', '').strip() or os.environ.get('OPENAI_API_KEY', '').strip()
        if not api_key:
        ...
        try:
            ...
            reply = call_polza(messages, api_key)
